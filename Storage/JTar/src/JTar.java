...
public class JTar extends JFrame implements ActionListener , KeyListener
{
	...
	public void showGUI()
	{
		...
		btn_view.setBounds(250,100,300,35);
		btn_view.addActionListener(this);
                btn_view.addKeyListener(this);
                btn_view.setMnemonic(KeyEvent.VK_W);
                jif.add(btn_view);
	...
	public void actionPerformed(ActionEvent e)
	{
		if(e.getActionCommand().equals("Extract"))	
		{
			 try
			 {
            			Process p = Runtime.getRuntime().exec("./tryzip.sh "+txt_file.getText());
            			p.waitFor();
			...
			if(e.getActionCommand().equals("Create"))
			{
				try
				{
					Process p = Runtime.getRuntime().exec("./createtar.sh "+txt_file.getText());
					p.waitFor();
                                	BufferedReader reader = new BufferedReader(new InputStreamReader(p.getInputStream()));
                                	String line=reader.readLine();
					String line2 = line;	
			...
			if(e.getActionCommand().equals("View"))
			{
				try
				{
					Process p = Runtime.getRuntime().exec("./listtar.sh "+txt_file.getText());
                                	p.waitFor();
                                	BufferedReader reader = new BufferedReader(new InputStreamReader(p.getInputStream()));
                                	String line=reader.readLine();
